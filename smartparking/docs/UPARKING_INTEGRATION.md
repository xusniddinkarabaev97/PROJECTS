...
```
   UParking                  SmartParking                Водитель
   ────────                  ────────────                ────────
   ...
   ③ Показывает QR ───────────────────────────────────►
                            ...
                            ⑦ Callback: статус оплачен
   ◄──────────────────────  POST {callbackUrl}
                            { paid: true }
```

1. UParking при выезде отправляет данные → SmartParking
2. SmartParking возвращает QR → UParking
3. UParking показывает QR водителю
4. Водитель сканирует QR → оплачивает через Click
5. Click подтверждает оплату → SmartParking
6. SmartParking отправляет статус "оплачено" → UParking (callback)
