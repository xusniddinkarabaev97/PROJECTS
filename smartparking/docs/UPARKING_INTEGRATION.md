...
```
┌──────────┐     ┌─────────────────┐     ┌──────────┐     ┌──────────┐     ┌──────────┐
│ UParking │────►│  SmartParking   │────►│   QR     │────►│  Click   │────►│ Шлагбаум │
│ (выезд)  │     │  /api/billing/  │     │ водителю │     │ (оплата) │     │ (Dahua)  │
└──────────┘     │     create      │     └──────────┘     └──────────┘     └──────────┘
                 └────────┬────────┘
                          │
                          ▼ callback
...
4. Click подтверждает оплату → SmartParking открывает шлагбаум
5. SmartParking отправляет статус оплаты → UParking (callback)
