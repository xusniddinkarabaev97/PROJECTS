...
const emptyPlan = { name: "", category: "", basePrice: "" };
...
export default function Plans() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (modal.type === "edit") {
        await api.updatePlan(modal.id, { ...payload, id: modal.id });
      } else {
  ...
  return (
    <div>
      <div className="card-header">
        <h2 style={{ fontSize: 24, fontWeight: 700, color: "var(--text-primary)" }}>📋 {t("plans")}</h2>
        ...
        <div className="card" style={{ padding: 0, overflow: "hidden" }}>
          <div style={{ overflowX: "auto" }}>
            <table className="data-table">
              <thead>
                <tr>
                  <th>ID</th><th>{t("name")}</th><th>{t("category")}</th><th>{t("basePrice")}</th><th>{t("actions")}</th>
                </tr>
              </thead>
              <tbody>
                {data.map((item) => (
                  <tr key={item.id}>
                    ...
                    <td>{item.category || "—"}</td>
                    <td style={{ fontWeight: 600 }}>{Number(item.basePrice || 0).toLocaleString()} UZS</td>
                    <td>
      ...
      {modal && (
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <h3 className="modal-title">{modal.type === "edit" ? t("edit") : t("add")} {t("plan")}</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <input className="input" placeholder={t("name")} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
              <input className="input" placeholder={t("category")} value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} />
              <input className="input" placeholder={t("basePrice")} type="number" value={form.basePrice} onChange={(e) => setForm({ ...form, basePrice: e.target.value })} />
