...
const emptyDepartment = {
  name: "", departmentType: "", location: "", headDoctor: "",
  companyId: 1, bedCount: "",
};
...
export default function Departments() {
  ...
  const openEdit = (item) => {
    setForm({
      ...
      bedCount: item.bedCount?.toString() || "",
    });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (modal.type === "edit") {
        await api.updateDepartment(modal.id, { ...payload, id: modal.id });
      } else {
  ...
  return (
    <div>
      <div className="card-header">
        <h2 style={{ fontSize: 24, fontWeight: 700, color: "var(--text-primary)" }}>🏥 {t("departments")}</h2>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p>{t("noDepartments")}</p>
          <button className="btn btn-primary" style={{ marginTop: 16 }} onClick={openAdd}>{t("addFirstDepartment")}</button>
        </div>
      ) : (
        <div className="card" style={{ padding: 0, overflow: "hidden" }}>
          <div style={{ overflowX: "auto" }}>
            <table className="data-table">
              <thead>
                <tr>
                  <th>ID</th><th>{t("name")}</th><th>{t("departmentType")}</th><th>{t("headDoctor")}</th><th>{t("location")}</th><th>{t("bedCount")}</th><th>{t("actions")}</th>
                </tr>
              </thead>
              <tbody>
                {data.map((item) => (
      ...
      {modal && (
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <h3 className="modal-title">{modal.type === "edit" ? t("editDepartment") : t("newDepartment")}</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <input className="input" placeholder={t("name")} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
              <input className="input" placeholder={t("departmentType")} value={form.departmentType} onChange={(e) => setForm({ ...form, departmentType: e.target.value })} />
              <input className="input" placeholder={t("location")} value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} />
              <input className="input" placeholder={t("headDoctor")} value={form.headDoctor} onChange={(e) => setForm({ ...form, headDoctor: e.target.value })} />
              <input className="input" placeholder={t("bedCount")} type="number" value={form.bedCount} onChange={(e) => setForm({ ...form, bedCount: e.target.value })} />
      ...
      {deleteId && (
        <div className="modal-overlay" onClick={() => setDeleteId(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p style={{ color: "var(--text-secondary)", marginBottom: 20 }}>{t("deleteDepartmentConfirm")}</p>
