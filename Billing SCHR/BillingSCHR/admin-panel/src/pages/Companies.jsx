...
const emptyCompany = { name: "", email: "", jwtAuthToken: "" };
  ...
  return (
    <div>
      <div className="card-header">
        <h2 style={{ fontSize: 24, fontWeight: 700, color: "var(--text-primary)" }}>🏢 {t("companies")}</h2>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p>{t("noCompanies")}</p>
          <button className="btn btn-primary" style={{ marginTop: 16 }} onClick={openAdd}>{t("addFirstCompany")}</button>
        </div>
      ) : (
        <div className="card" style={{ padding: 0, overflow: "hidden" }}>
          <div style={{ overflowX: "auto" }}>
            <table className="data-table">
              <thead>
                <tr>
                  <th>ID</th><th>{t("name")}</th><th>{t("email")}</th><th>{t("token")}</th><th>{t("actions")}</th>
                </tr>
              </thead>
              <tbody>
                {data.map((item) => (
      ...
      {modal && (
        <div className="modal-overlay" onClick={() => setModal(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <h3 className="modal-title">{modal.type === "edit" ? t("editCompany") : t("newCompany")}</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <input className="input" placeholder={t("name")} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />
              <input className="input" placeholder={t("email")} type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />
              <input className="input" placeholder={t("password")} type="password" value={form.jwtAuthToken} onChange={(e) => setForm({ ...form, jwtAuthToken: e.target.value })} />
      ...
      {deleteId && (
        <div className="modal-overlay" onClick={() => setDeleteId(null)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p style={{ color: "var(--text-secondary)", marginBottom: 20 }}>{t("deleteCompanyConfirm")}</p>
